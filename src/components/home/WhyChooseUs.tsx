...
export default function WhyChooseUs() {
  ...
  return (
    <section className="py-24 bg-sacred-dark text-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-saffron-500/30 bg-saffron-500/10 text-saffron-400 text-xs uppercase tracking-[0.2em] font-semibold mb-4">
            <Award size={14} />
            Why Choose Us
          </span>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {reasons.map((reason, i) => (
            <div
              key={i}
              className="bg-white/5 backdrop-blur-md rounded-3xl p-8 border border-white/10 hover:bg-white/10 hover:border-saffron-500/30 hover:-translate-y-2 transition-all duration-300 group shadow-lg"
            >
              <div className="w-14 h-14 rounded-2xl bg-saffron-500/10 flex items-center justify-center mb-6 group-hover:bg-saffron-500/20 group-hover:scale-110 transition-all duration-300">
                <reason.icon size={28} className="text-saffron-400" strokeWidth={1.5} />
              </div>
              <h3 className="font-playfair text-xl font-bold text-white mb-3">
                {reason.title}
              </h3>
              <p className="text-white/50 text-sm leading-relaxed font-light">{reason.desc}</p>
            </div>
          ))}
        </div>
      </div>
