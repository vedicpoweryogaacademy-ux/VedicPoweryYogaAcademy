...
export default function TrustBadges() {
  ...
  return (
    <section className="bg-sacred-dark py-14 border-t border-saffron-500/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <p className="text-center text-saffron-400/50 text-xs uppercase tracking-[0.2em] font-semibold mb-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5">
          {badges.map((badge, i) => (
            <div
              key={i}
              className={`bg-gradient-to-br ${badge.color} rounded-2xl p-5 text-center border border-white/5 hover:border-saffron-500/30 transition-all duration-300 hover:-translate-y-1 group`}
            >
              ...
              <p className="text-white/40 text-xs mt-1.5">{badge.subtitle}</p>
            </div>
          ))}
        </div>
      </div>
